...
To aid the planning of my project, I first broke down the game into smaller parts eg. planning the HTML, JavaScript and CSS aspects. As shown in the image, I used GoogleDocs and drawings to pseudocode the project. This meant that before I started coding, I understood the different data types, DOM elements and classes that I would need to implement in order for my project to be successful. Although I was eager to start coding, this process demonstrated to me the importance of planning before starting projects. By questioning my logic before I started and discussing and researching solutions, I was able to code more fluently and face less issues than if I had started coding immediately. 
...
The game is played on a grid. To maximise efficiency whilst making this grid, I used HTML to create 10 divs (for each cell of the grid), then I used a JavaScript `for` loop to populate the DOM with a 10 x 10 grid comprised of cells. Each of these cells were 10% of the grid height and width and styled with CSS. Each cell has an index value between 0-99 - this meant I could access the positions of the snake and the food which was necessary to determine the functionality of the game. 
...
This key down events are only activated if the snake is currently moving left or right. When activated, the previous `set interval` is cleared (this prevents multiple set intervals running simultaneously). Alternatively, when the snake is moving up/down the board, only the right/left arrow keys are activated. 
...
This project gave me the opportunity to combine everything I had learnt in the first 3 weeks of this course into one project. I became more confident at learning how to debug code, using CSS animations and working with the DOM to provide interactive webpages that respond to the user. Additionally, I learnt the importance of testing every time you add additional features and the importance of using `console.log` to debug code. I developed my ability to plan a project and work to meet deadlines. Furthermore, I learnt how to present and explain code to the other members of my cohort. 

# Bugs

- The snake is only able to move right/up/down at the start. To combat this in the future, I may add a default initial movement which causes the snake to move to the right when the user clicks 'play'. 
